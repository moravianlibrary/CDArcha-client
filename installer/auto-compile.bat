...
"%InnoDir%\Compil32.exe" /cc cdarcha_installer.iss
REM Compile NoAdmin setup file
"%InnoDir%\Compil32.exe" /cc cdarcha_installer_noAdmin.iss
REM count SHA256 of Admin installer
for /f "tokens=1 delims= " %%a in ('sha256deep.exe Output/cdarcha_setup.exe') do @set hashAdmin=%%a
REM count SHA256 of NoAdmin installer
for /f "tokens=1 delims= " %%a in ('sha256deep.exe Output/cdarcha_setupNoAdmin.exe') do @set hashNoAdmin=%%a
REM get Version
for /f "tokens=1-2" %%i in ('sigcheck.exe Output\cdarcha_setup.exe /accepteula') do ( if "%%i"=="Version:" set filever=%%j )
